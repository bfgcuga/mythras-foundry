...
export async function openFatigueCheckDialog() {
  ...
  const result = await foundry.applications.api.DialogV2.wait({
    ...
    content: `<div class="mythras-foundry mythras-dialog"><fieldset><legend>${escape(game.i18n.localize("MYTHRASF.FatigueCheck.Group"))}</legend><select name="party" class="sheet-field-editable">${partyOptions}</select></fieldset><fieldset><legend>${escape(game.i18n.localize("MYTHRASF.FatigueCheck.Participants"))}</legend><div data-fatigue-members></div></fieldset><fieldset><legend>${escape(game.i18n.localize("MYTHRASF.FatigueCheck.RollConfiguration"))}</legend><label><span>${escape(game.i18n.localize("MYTHRASF.FatigueCheck.SkillLabel"))}</span><select name="skill" class="sheet-field-editable">${skillOptions}</select></label><label><span>${escape(game.i18n.localize("MYTHRASF.FatigueCheck.Difficulty"))}</span><select name="difficulty" class="sheet-field-editable">${difficultyOptions}</select></label></fieldset></div>`,
